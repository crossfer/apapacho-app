...
// Calendar-specific palette, as specified for this view. Note this does NOT
// match the shared STATUS_COLOR map in lib/constants.ts (used by
// StatusBadge elsewhere) — scheduled/in_progress/completed use different
// brand colors here, and cancelled uses a non-brand gray to de-emphasize it.
// Status colors will look different on the calendar vs. everywhere else.
const CALENDAR_STATUS_COLOR: Record<ServiceStatus, string> = {
  ...
  // Granular repair-flow statuses aren't produced by the general Orders
  // calendar, but ServiceStatus includes them — map to a reasonable parent.
  received: '#4F6D5A',
  diagnosing: '#B68A4C',
  in_repair: '#B68A4C',
  ready: '#B83E7A',
    ...
    <div className="apapacho-calendar rounded-2xl bg-white p-4 shadow-sm">
      <FullCalendar
        ...
        eventClick={(info: EventClickArg) => {
          ...
          router.push(`/admin/ordenes/${info.event.id}`)
        }}
      />
    </div>
  )
